...
export const environment = {
  ...
  IdentityAppId: process.env.MANAGE_CLIENT_ID, /* The Cognito User Pool App Id */
  Region: process.env.AWS_REGION, /* The AWS Region where the Cognito and API is deployed */
  IdentityAppDomain: process.env.MANAGE_DOMAIN, /* The domain name for the app (include port number for localhost) */
  ...
  forgotPasswordUrl: `https://${process.env.COGNITO_DOMAIN}/forgotPassword?response_type=token&client_id=${process.env.MANAGE_CLIENT_ID}&redirect_uri=https://${process.env.MANAGE_DOMAIN}/dashboard&state=STATE&scope=openid+profile+aws.cognito.signin.user.admin+email`,
  signUpUrl: '', /* The url to redirect to when signing up */
  googleLogin: `https://${process.env.COGNITO_DOMAIN}/oauth2/authorize?identity_provider=Google&redirect_uri=https://${process.env.MANAGE_DOMAIN}/dashboard&response_type=TOKEN&client_id=${process.env.MANAGE_CLIENT_ID}&state=STATE&scope=openid profile aws.cognito.signin.user.admin email`,
  cognitoClientId: process.env.BEHAVIOR_CLIENT_ID,
  cognitoCallbackUrl: `https://${process.env.MANAGE_DOMAIN}/dashboard`, /* The callback url after the user signs in (example: https://localhost:8000 ) */
