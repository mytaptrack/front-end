...
interface ProcessingReportData {
  behavior: typesV2.StudentBehavior;
  source: {
    device: string;
    rater?: string;
  };
...
export class ChartUtilsService {
  ...
  createChartData(report: typesV2.ReportDefinition, reportData: typesV2.ReportDetails, behaviors: typesV2.StudentBehavior[], maxDays: number = 7, showTargets: boolean = true, trackedDaysPerWeek: number = 5, startDate: moment.Moment = null, errors: string[] = []): ChartConfiguration {
...
//        plugins,
  ...
  private getBubbleChartData(report: typesV2.ReportDefinition, reportData: typesV2.ReportDetails, behaviors: typesV2.StudentBehavior[], startDate: moment.Moment, maxDays: number): ReportDetails {
    return this.getClusterChartData(report, reportData, behaviors, startDate, maxDays, 10);
  }
  
  private getClusterChartData(report: typesV2.ReportDefinition, reportData: typesV2.ReportDetails, behaviors: typesV2.StudentBehavior[], startDate: moment.Moment, maxDays: number, pointsPerHour: number = 120): ReportDetails {
  ...
  public getDurationData(data: typesV2.ReportData[], startDate: moment.Moment, maxDays: number): {minutes: number, duration: number}[] {
    if(data.length == 0) {
      return [];
    }
  ...
  private getDurationChart(report: typesV2.ReportDefinition, reportData: typesV2.ReportDetails, behaviors: typesV2.StudentBehavior[], startDate: moment.Moment, maxDays: number, errors: string[]): ReportDetails {
    let labels: string[] = [];
  ...
  private getBarChartData(report: typesV2.ReportDefinition, reportData: typesV2.ReportDetails, behaviors: typesV2.StudentBehavior[], maxDays: number, startDate: moment.Moment): ReportDetails {
    let data: typesV2.ReportData[] = reportData.data
    ...
    const behaviorMap: {[key: string]: typesV2.StudentBehavior } = {}
  ...
  private constructReportData(reportData: typesV2.ReportDetails, behaviors: typesV2.StudentBehavior[], errors: string[]): ProcessingReportData[] {
  ...
  private getLineChartData(report: typesV2.ReportDefinition, reportResponse: typesV2.ReportDetails, behaviors: typesV2.StudentBehavior[], startDate: moment.Moment, maxDays: number, showTargets: boolean, trackedDaysPerWeek: number, errors: string[]): ReportDetails {
    const labels: string[] = [];
    const datasets: ChartDataset[] = [];
    let minValue = 0;
    let maxValue = 3;
    ...
    try {
      for (let i = 0; i < report.metrics.length; i++) {
        let item: typesV2.ReportDefinitionMetric = JSON.parse(JSON.stringify(report.metrics[i]));
        item.name = this.getMetricName(item, behaviors);
        ...
        if(item.timeline.calculationType == typesV2.CalculationType.avg) {
  ...
  public getIntervalModel(report: typesV2.ReportDetails, dayStart: moment.Moment, minutesPerRange: number, intervalType: 'minutes' | 'seconds', studentBehaviors: typesV2.StudentBehavior[]): IntervalModel {
  ...
  public getMetricName(metric: typesV2.ReportDefinitionMetric, behaviors: typesV2.StudentBehavior[]) {
