...
import { 
  AbcCollection, ActivityGroupDetails, ActivityGroupSummary, ApplyLicenseRequest, AppPutRequest, BehaviorSubscription, 
  DeleteDeviceRequest, DevicePutRequest, DeviceRegisterPutRequest, EfficacyPostRequest, GetLicenseTemplatesResponse, 
  IoTAppDevice, IoTDevice, LicenseDetails, LicenseDetailsWithUsage, LicenseDisplayTagsPut, LicenseStats, 
  LicenseStudentTemplateDelete, LicenseStudentTemplatePut, ManageAppRenamePostRequest, ManageReportDateDataPoint, 
  ManageReportOffsetDataPoint, ManageReportPostResponse, ManageStudentGetResponse, MobileDevice, NotificationDetailsBehavior, 
  NotificationDetailsTeam, OverwriteScheduleDeleteRequest, OverwriteSchedulePutRequest, PutDocumentRequest, PutSettingsRequest, 
  QLStudent, QLStudentNote, QLStudentNoteRequest, QLStudentUpdateInput, ReportData, ReportDetails, ScheduleCategory, 
  ScheduleDeleteRequest, SchedulePutRequest, Student, StudentBehavior, StudentBulkPut, StudentCreateRequest, 
  StudentDashboardSettings, StudentDataExcludeRequest, StudentDataPut, StudentDocument, StudentExcludeIntervalPutRequest, 
  StudentNotesPut, StudentReportPostRequest, StudentResponse, StudentResponsePutRequest, StudentSubscriptions, StudentSummary, 
  StudentSummaryReport, StudentSummaryStats, StudentTrackPut, StudentTrackStateGetResponse, SubscriptionDeleteRequest, 
  SubscriptionPostRequest, TeamDeleteRequest, TeamPostRequest, TeamPutRequest, TrackDeviceActionRequest, TrackDeviceTermStatus, 
  User, UserContext, UserPutRequest, UserSubscriptionResponse, UserSummary, UserSummaryRestrictions, UserSummaryStatus, Notification
} from '@mytaptrack/types';
import { post } from 'aws-amplify/api';
import { del, generateClient, get, GraphQLQuery, GraphQLSubscription, put } from '@aws-amplify/api';
...
export class ApiClientService {
  ...
  async putReport(request: StudentSummaryReport) {
    request.behaviors.forEach(x => delete x.stats);
  ...
  async getNotes(studentId: string, date: Moment): Promise<QLStudentNote[]> {

    const result = await this.gqlQuery<QLStudentNote[]>(`
      query getSnapshot($endDate: String!, $startDate: String!, $studentId: String!) {
      ...
      `,
      {
          ...
          endDate: date.clone().format('yyyy-MM-DD')
      }, 'getNotes');
  ...
  async putStudentDataV2(event: StudentDataPut) : Promise<ReportData> {
    return await this.put('v2/reports/data', event);
  }
  ...
  private async put(subpath: string, content: any): Promise<any> {
    if(!this.auth.token) {
      return;
    }
  ...
  private async post<T>(subpath: string, content: any) {
    if(!this.auth.token) {
      return;
    }
    let retry = 0;
