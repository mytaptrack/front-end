...
import { 
  AppClass, IoTDeviceClass, IoTDeviceCollection, StudentClass 
} from '../../../lib';
import { IoTDeviceEvent, 
  StudentBehavior, StudentResponse } from '@mytaptrack/types';
...
export class DeviceAppComponent implements OnInit {
  ...
  changeOrder(behavior: DeviceBehavior, amount: number) {
    behavior.event.order += amount;
    this.resortActiveBehaviors(behavior);
  }
  ...
  toggleAlertForApp(behavior: StudentBehavior) {
    if(!this.selected || !this.selected.isApp) {
      return false;
    }

    const event = this.selected.events.find(x => x.eventId === behavior.id);
    if(event) {
      event.alert = !event.alert;
    } else {
      this.selected.events.push({
        eventId: behavior.id,
        track: false,
        alert: true,
        title: behavior.name,
        isDuration: behavior.isDuration,
        order: this.selected.events.length
      } as any);
    }
  }
  ...
  toggleBehaviorForApp(behavior: StudentBehavior) {
    ...
    if(event.track && this.student.trackables.responses) {
      const response = this.student.trackables.responses.find(x => x.id == event.eventId);
      if(response.appliedToBehaviors.length > 0) {
        const behavior = this.student.trackables.behaviors.find(x => response.appliedToBehaviors.find(y => x.id == y.behaviorId));
        if(behavior.requireResponse) {
          event.alert = true;
        }
      }
    }
  ...
  isAlertCheckedForApp(behavior: StudentBehavior): boolean {
    if(!this.selected) {
      return false;
    }

    return this.selected.events.find(x => x.eventId === behavior.id && x.alert)? true : false;
  }
