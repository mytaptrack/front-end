...
import { MobileDevice, MobileDeviceRegistration, TrackedBehavior } from "@mytaptrack/types";
...
export class ManageAppClass {
    ...
    async addAssignment(studentId: string, fromAssignment: ManageAppAssignment) {
        ...
        const result = await this.api.putStudentAppV2({ 
            studentId: studentId,
            deviceId: this.device?.id ?? this.data.device.id,
            deviceName: this.device?.name ?? this.data.device.name,
            dsn: '',
    ...
    async reassign() {
        ...
        try {
            await this.api.postUpdateManagedApp({
                tags: this.tags,
                name: this.device?.name ?? this.data.device.name,
                deviceId: this.device?.id ?? this.data.device.id,
                license: this.license,
                reassign: true
            });
...
export class ManageAppsClass {
    ...
    async create() {
        ...
        await this.api.postUpdateManagedApp(params);
