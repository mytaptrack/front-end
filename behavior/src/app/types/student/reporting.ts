...
import {
    StudentDashboardSettings, StudentClass,
    Student, ReportData, ReportDetails, SummaryScope, CalculationType,
    BehaviorSettings, MetricType, ScheduleClass, StudentSummaryReport, AccessLevel,
    moment,
    QLStudentNote
} from '..';
import { colors } from '../../components/student/dashboard2/dashboard2.component';
...
export class StudentReportClass {
    ...
    async getNotes(date: moment.Moment, endDate: moment.Moment = undefined) {
        let finalDate = endDate ?? date.clone().add(1, 'day');
        return this.api.getQLNotes(this.student.studentId, date.format('yyyy-MM-DD'), finalDate.format('yyyy-MM-DD'));
    }
    async saveNotes(note: QLStudentNote) {
        await this.api.putQLNotes(note);
    }
        ...
        settings.behaviors = [].concat(
            ...student.behaviors
                ...
                .map(b => {
                    ...
                    const color = colors.find(x => !settings.behaviors.find(y => y.frequency == x[0]));
                    const retval = {
                        id: b.id,
                        frequency: color[0],
                        expanded: false,
