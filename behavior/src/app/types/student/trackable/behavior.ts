...
import {
    StudentBehavior, StudentClass,
    StudentDataPut, moment,
    ReportData, TargetsClass
} from '../..';
    ...
    async save() {
        ...
        try {
            ...
            if(!this.id) {
                ...
                const result = await this.student.saveGraphQL();
                const resultBehavior = result.behaviors.find(x => x.name == this.name);
                this.id = resultBehavior.id;
                this.student.trackables.refreshBehaviorViews();
            } else {
                await this.student.saveGraphQL();
            }
        } finally {
            this.setSaving(true);
        }
